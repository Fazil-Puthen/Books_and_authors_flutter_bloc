...
class AuthorBloc extends Bloc<AuthorEvent, AuthorState> {
  ...
  //fetch all authors
   FutureOr<void> fetchauthors(FetchAuhtors event, Emitter<AuthorState> emit)async {
    emit(AuthorLoading());
    try{
  ...
  FutureOr<void> addauthor(Addauthor event, Emitter<AuthorState> emit)async {
    emit(AuthorLoading());
    final random=Random();
    String randomId = List.generate(12, (index) => random.nextInt(256).toRadixString(16).padLeft(2, '0')).join();
