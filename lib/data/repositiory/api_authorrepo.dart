...
import 'package:elt_machine_task/domain/repositiory/author_repo.dart';
...
class Author implements AuthorRepo{
  ...
  final url=Uri.parse(ApiConstants.baseurl+ApiConstants.author);
  try{
    ...
    if(response.statusCode==200){
      ...
      auhtorlist=result.map((element)=>AuthorModel.fromjson(element)).toList();
  ...
  @override
  Future<Either<String,List<AuthorModel>>> createauthor(AuthorModel author) async{
    //url
   final url=Uri.parse(ApiConstants.baseurl+ApiConstants.author);
     try{
